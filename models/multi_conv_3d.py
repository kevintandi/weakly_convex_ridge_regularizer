...
class MultiConv3d(nn.Module):
    ...
    def spectral_norm(self, mode="Fourier", n_steps=1000):
        ...
        if mode =="Fourier":
            ...
            ft = torch.fft.fftn(torch.nn.functional.pad(kernel, (padding, padding, padding, padding, padding, padding)), dim=(2, 3, 4))
    ...
    def init_id(self, n_iter = 1000):
        #
        with torch.no_grad():
            L = self.spectral_norm(mode="Fourier")
            for conv in self.conv_layers:
                 conv.weight.data *= L**(1/(len(self.conv_layers)))
        self.dirac = self.dirac.to(self.conv_layers[0].weight.device)
        optimizer = torch.optim.Adam(self.parameters(),  0.05)
        pbar = tqdm(range(n_iter))
        for i in pbar:
            optimizer.zero_grad()
            loss = torch.sum((self.transpose(self.convolution(self.dirac)) - self.dirac)**2)
            loss.backward()
...
class ZeroMean3d(nn.Module):
    def forward(self, X):
        Y = X - torch.mean(X, dim=(1,2,3,4)).unsqueeze(1).unsqueeze(2).unsqueeze(3).unsqueeze(4)
        return(Y)
