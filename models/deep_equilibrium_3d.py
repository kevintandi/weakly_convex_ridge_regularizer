...
class DEQFixedPoint(nn.Module):
    def __init__(self, model, params_fw, params_bw):
        ...
        self.backward_niter = 0
        self.f = None
        self.fjvp = None
        self.keep_mean = False
    ...
    def forward(self, x_noisy, sigma=None, **kwargs):
        ...
        if self.keep_mean:
            mean = torch.mean(x_noisy, dim=(1, 2, 3, 4)).view(-1, 1, 1, 1, 1)
...
def anderson(f, x0, m=5, lam=1e-4, max_iter=50, tol=1e-5, beta = 1.0):
    ...
    bsz, d, H, W, D = x0.shape
    X = torch.zeros(bsz, m, d*H*W*D, dtype=x0.dtype, device=x0.device)
    F = torch.zeros(bsz, m, d*H*W*D, dtype=x0.dtype, device=x0.device)
