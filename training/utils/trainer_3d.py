...
sys.path.append('..')
from models import deep_equilibrium_3d
...
class Trainer:
    ...
    def __init__(self, config, seed, device):
        ...
        self.model, self.config = build_model(self.config, dims=3)
        self.model = self.model.to(device)
        #self.model.conv_layer.init_id(n_iter=10000)
        ...
        # Set the DEQ solver
        self.denoise = deep_equilibrium_3d.DEQFixedPoint(self.model, self.config["training_options"]["fixed_point_solver_fw_params"], self.config["training_options"]["fixed_point_solver_bw_params"])
    ...
    def valid_epoch(self):
        ...
        with torch.no_grad():
            ...
            for batch_idx, data in enumerate(tbar_val):
                ...
                sigma = self.noise_val * torch.torch.ones((data.shape[0], 1, 1, 1, 1), device=data.device)
                noise = sigma / 255 * torch.randn(data.shape,device=data.device)
                noisy_data = data + noise
